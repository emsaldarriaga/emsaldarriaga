...
function generateCanonicalURL(fileData, config) {
  'use strict'; //const datePart: Array<string> = fileData.name.split('-')
  //datePart.length = 3

  const titlePart = fileData.name.split('-').slice(3).join('-'); //return config.site.url.replace(/\/$/, '') + (datePart.join('/') + '/' + titlePart + '.html')
  // edit: doing away with datePart and formating url to be title only

  return config.site.url.replace(/\/$/, '') + '/' + titlePart + '.html';
}
/**
 * Generate postid for the post (needed for disqus)
 * 
...
function convertDateFormat(postData, pathToFile, months, lang = 'en') {
  ...
  if (months.length !== 12) {
    ...
    const dateString = postData.attributes.date.toString()
  const dateParts = dateString.split('-'); // replace month number with month name
...
function saveBlogpostsHTML(fileData, destPath, layoutContent) {
  ...
  datePart.length = 3;
  const titlePart = fileData.name.split('-').slice(3).join('-'); // year/month/day/post_title.html
  // edit out datePart and replace with /projects
  //const fileName: string = datePart.join('/') + '/' + titlePart + '.html'

  const fileName = 'projects' + '/' + titlePart + '.html'; // saves the blogposts' html file to public/year/month/day/post_title.html
