(function () {
  ...
  if (nodeEnvironment === 'production') {
    ...
    const csp = require('helmet-csp'); // These settings should be changed (these here are just examples)!


    app.use(csp({
      directives: {
        defaultSrc: [`'none'`],
        styleSrc: [`'self'`, 'https://fonts.googleapis.com', 'https://www.youtube.com', 'https://maxcdn.bootstrapcdn.com/', '//cdnjs.cloudflare.com'],
        fontSrc: [`'self'`, 'https://fonts.gstatic.com', 'https://maxcdn.bootstrapcdn.com'],
        scriptSrc: [`'self'`, 'https://www.youtube.com', 'https://www.googletagmanager.com', 'https://www.google-analytics.com', 'https://code.jquery.com', 'https://maxcdn.bootstrapcdn.com', '//cdnjs.cloudflare.com'],
        childSrc: [`'self'`, 'https://www.youtube.com'],
        imgSrc: [`'self'`, 'www.google-analytics.com', 'https://use.fontawesome.com', 'https://cloud.netlifyusercontent.com'],
        objectSrc: [`'none'`],
        connectSrc: [`'self'`]
      }
    }));
  ...
  app.use('/', favicon(path.join('public', 'favicon.ico'))); // routes for all publications page

  app.get('/publications', (req, res) => {
    //	res.sendFile(path.join('public', '/publications.html'));
    res.sendFile('publications.html', {
      root: './public'
    });
  }); //
  ...
  app.get('/north-america', (req, res) => {
    res.sendFile("north-america.html", {
      root: './public'
    });
  });
  app.get('/latin-america', (req, res) => {
    res.sendFile("latin-america.html", {
      root: './public'
    });
  });
  app.get('/sub-saharan-africa', (req, res) => {
    res.sendFile("sub-saharan-africa.html", {
      root: './public'
    });
  }); //
